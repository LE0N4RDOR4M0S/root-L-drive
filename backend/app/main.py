...
async def lifespan(_: FastAPI):
	...
	try:
		...
		await db["machines"].create_index([("owner_id", 1), ("created_at", -1)])
		await db["machines"].create_index([("token_hash", 1)], unique=True)
		await db["machines"].create_index([("owner_id", 1), ("is_active", 1)])
...
app.include_router(__import__("app.routes.machines", fromlist=["router"]).router, prefix=settings.api_prefix)
